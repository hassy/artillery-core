...
SocketIoEngine.prototype.createScenario = function(scenarioSpec, ee) {
  ...
  let tasks = _.map(scenarioSpec.flow, function(rs) {
    ...
    return self.step(rs, ee, {
      beforeMessage: scenarioSpec.beforeMessage || [],
      afterMessage: scenarioSpec.afterMessage || []
    });
  });
...
SocketIoEngine.prototype.step = function (requestSpec, ee, opts) {
  ...
  let f = function(context, callback) {
    ...
    let functionNames = _.concat(opts.beforeMessage || [], requestSpec.emit.beforeMessage || []);
    let outgoing = {
    ...
    async.eachSeries(
      ...
      function iteratee(functionName, next) {
        ...
        let processFunc = self.config.processor[functionName];
        processFunc(outgoing.data, context, ee, function (err) {
          if (err) {
            return next(err);
          }
          return next(null);
      ...
      function done(err) {
        ...
        if (isResponseRequired(requestSpec)) {
          ...
          context.socketio.on(response.channel, function receive(data) {
            ...
            // Run afterMessage functions:
            let functionNames = _.concat(opts.afterMessage || [], requestSpec.emit.response.afterMessage || []);
            async.eachSeries(
              functionNames,
              function iteratee(functionName, next) {
                let processFunc = self.config.processor[functionName];
                processFunc(outgoing.data, data, context, ee, function (err) {
                  if (err) {
                    return next(err);
                  }
                  return next(null);
              ...
              function done(err) {
                ...
                processResponse(ee, data, response, context, function(err2) {
                  if (!err2) {
