...
function runScenario(script, intermediate, aggregate) {
  ...
  if (!compiledScenarios) {
    ...
    scenarioEvents.on('customStat', function(stat) {
      ...
      iBag.histogram(stat.stat, stat.value);
      aBag.histogram(stat.stat, stat.value);
    });
    ...
    scenarioEvents.on('error', function(errCode) {
      ...
      iBag.group('errors', errCode);
      aBag.group('errors', errCode);
    });
    ...
    scenarioEvents.on('match', function() {
      ...
      aggregate.addMatch();
      iBag.counter('matches');
      aBag.counter('matches');
    });
    scenarioEvents.on('response', function(delta, code, uid) {
      ...
      iBag.histogram('latency', delta);
      iBag.group('codes', code);
      aBag.histogram('latency', delta);
      aBag.group('codes', code);
    });
  ...
  compiledScenarios[i](scenarioContext, function(err, context) {
    ...
    if (err) {
      ...
      iBag.counter('scenariosCompleted');
      aBag.counter('scenariosCompleted');
    }
  });
}
